...
export function handleCompositionEndOnCellTemplate(state: State, event: KeyboardEvent): State {
    ...
    if (cellTemplate.handleCompositionEnd && !state.currentlyEditedCell) { // TODO need add !(event.shiftKey && event.keyCode === keyCodes.SPACE) to working keycodes (shift + space) in a lower condition
        const { cell: newCell, enableEditMode } = cellTemplate.handleCompositionEnd(cell, (event as any).data);
