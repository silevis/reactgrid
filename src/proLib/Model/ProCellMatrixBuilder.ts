...
interface ProStickyRanges extends StickyRanges {
  stickyRightRange: Range;
  stickyBottomRange: Range;
}

export interface ProCellMatrixProps extends CellMatrixProps {
  stickyRightColumns?: number;
  stickyBottomRows?: number;
}

interface ProStickyEdges extends StickyEdges {
  rightStickyColumns: number;
  bottomStickyRows: number;
}

export type ProCellMatrix = CellMatrix<ProStickyRanges, ProCellMatrixProps>;

export class ProCellMatrixBuilder
  implements ICellMatrixBuilder<ProCellMatrixBuilder, ProStickyEdges>
{
  private cellMatrix!: ProCellMatrix;

  constructor() {
    this.reset();
  }

  private reset(): ProCellMatrixBuilder {
    this.cellMatrix = new CellMatrix({} as ProStickyRanges);
    return this;
  }

  setProps(props: ProCellMatrixProps): ProCellMatrixBuilder {
    this.cellMatrix.props = props;
    return this;
  }

  fillRowsAndCols(
    edges: ProStickyEdges = {
      leftStickyColumns: 0,
      topStickyRows: 0,
      rightStickyColumns: 0,
      bottomStickyRows: 0,
    }
  ): ProCellMatrixBuilder {
    const {
      leftStickyColumns,
      topStickyRows,
      rightStickyColumns,
  ...
  setRangesToRenderLookup(): ProCellMatrixBuilder {
  ...
  setEdgeLocations(): ProCellMatrixBuilder {
