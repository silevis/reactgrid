...
To enable cut, copy and paste you need to pass following handlers into ReactGrid component
...
<LiveCode
  ...
  code={`
    ...
    const handleCut = (event, cellsRange: NumericalRange, cellsLookup: CellsLookup) => {
      ...
      let htmlData = "<table>";
      for (let rowIndex = 0; rowIndex < cellsRange.endRowIdx - cellsRange.startRowIdx; rowIndex++) {
        htmlData += "<tr>";
        for (let colIndex = 0; colIndex < cellsRange.endColIdx - cellsRange.startColIdx; colIndex++) {
    ...
    const handleCopy = (event, cellsRange: NumericalRange, cellsLookup: CellsLookup) => {
      const { startRowIdx, endRowIdx, startColIdx, endColIdx } = cellsRange;
      const cellsLookupCallbacks: CellsLookupCallbacks[] = [];
    ...
    const handlePaste = (event, cellsArea: NumericalRange, cellsLookup: CellsLookup) => {
        ...
        rows.forEach((row, rowIndex) => {
          ...
          cells.forEach((cell, colIndex) => {
            ...
            const gridCell = cellsLookup.get(
              (cellsArea.startRowIdx + rowIndex) + " " + (cellsArea.startColIdx + colIndex)
            );
            if (gridCell) {
