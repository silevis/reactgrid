...
Below is an example with overridden logic for cut, copy, and paste. Additionally, `handlePaste` utilizes the `setSelectedArea` method from the gridAPI to highlight the new cell area after pasting the data.
