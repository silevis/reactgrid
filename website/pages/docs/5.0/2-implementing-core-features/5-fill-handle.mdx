...
```ts
/**
 * Callback function that is triggered when the fill handle is used.
 * 
 * @param selectedArea - The range of cells that were initially selected.
 * @param fillRange - The range of cells that the fill handle is dragged over.
 * @param cellsLookup - A lookup Map to get and set cell values.
 * @returns void
 */
onFillHandle?: (selectedArea: NumericalRange, fillRange: NumericalRange, cellsLookup: CellsLookup) => void;

interface CellsLookupCallbacks {
  rowIndex: number;
  colIndex: number;
  onStringValueRequsted: () => string;  // <-- getting cell value
  onStringValueReceived: (v: string) => void; // <-- setting cell value
}
