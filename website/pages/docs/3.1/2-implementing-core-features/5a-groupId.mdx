...
by marking cells on `groupId` property using [`Id`](/docs/3.1/7-api/1-types/4-id/) interface.
...
`groupId` property is available in every cell template because it's provided by [`Cell`](/docs/3.1/7-api/0-interfaces/4-cell/)
interface. 

import LiveCode from "@/nextra/components/LiveCode.tsx";
import { ReactGrid, Column, Row, CellChange, MenuOption, SelectionMode, Id } from "@silevis/reactgrid-3.1";
