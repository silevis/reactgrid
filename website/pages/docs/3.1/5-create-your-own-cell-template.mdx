...
For this tutorial we are going to use the previous project - [handling changes](/docs/3.1/2-implementing-core-features/1-handling-changes/).
...
* `getCompatibleCell` as a parameter gets an incoming cell and returns a compatible cell (with `text` and `value`
  attribute required by [`Compatible`](/docs/3.1/7-api/1-types/1-compatible-cell/) type). 
...
* `update` - as we are not sure if an incoming cell has the same interface like `FlagCell` so we mark it
  as `UncertainCompatible` (the incoming cell has attributes provided by [`Compatible`](/docs/3.1/7-api/1-types/1-compatible-cell/) 
...
This record tells us that cells rows can be on one of [`DefaultCellTypes`](/docs/3.1/7-api/1-types/6-default-cells/) (build-in cell types) or brand new 
