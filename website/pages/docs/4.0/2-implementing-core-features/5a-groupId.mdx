...
by marking cells on `groupId` property using [`Id`](/docs/4.0/7-api/1-types/4-id/) interface.
...
import { ReactGrid, Column, Row, CellChange, MenuOption, SelectionMode, Id } from "@silevis/reactgrid-4";
import "@silevis/reactgrid-4/styles.css";

`groupId` property is available in every cell template because it's provided by [`Cell`](/docs/4.0/7-api/0-interfaces/4-cell/)
interface. 
