...
For this example, let's create a configuration file, such as `reactgrid.config.ts`, to store the data that will be used by ReactGrid. In many cases, however, the data is often fetched from a server.
...
import { peopleArr } from "./utils/examplesConfig";
