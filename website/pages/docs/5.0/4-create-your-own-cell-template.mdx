...
In this example, we will create a custom `DateCell` template:
...
export const DateCell: FC<DateCellProps> = ({ value, onValueChanged }) => {
  ...
  return (
    <CellWrapper
      onStringValueRequsted={() => value.toDate().toDateString()}
