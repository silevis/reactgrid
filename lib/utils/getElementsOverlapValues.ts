export function getElementsOverlapValues(firstEl: HTMLElement, secondEl: HTMLElement, negativeValues?: boolean) {
  ...
  if (negativeValues) {
    ...
    overlapX = Math.max(0, Math.min(firstRect.right, secondRect.right) - Math.max(firstRect.left, secondRect.left));
    overlapY = Math.max(0, Math.min(firstRect.bottom, secondRect.bottom) - Math.max(firstRect.top, secondRect.top));
  }
